...
- 見た目だけの装飾物(木・井戸・灯籠・柵・花)は `decorations: DecorationPlacement[]` として地形とは別に持ち、
...
- **キャスト**: 「飛距離ゲージ」（三角波で0↔100を往復するバーを狙ったタイミングで止める）操作。
  `game/fishing.ts` の `castGaugePosition`(経過時間→ゲージ位置) / `castQualityFromPosition`
  (中央=50に近いほど質が高い) / `castQualityBiteMultiplier`(質をアタリ待ち時間の短縮倍率に変換)
  が判定ロジックを担い、`FishingScreen.tsx` はゲージバーの表示と停止操作のみを行う。
- **待機**: アタリまでの待ち時間は `spots.json` の待機時間レンジと餌/仕掛けの倍率、
  さらにキャストの質(`castQualityBiteMultiplier`)で決まる（既存の `rollBiteWaitMs` を流用し、
  質の倍率で除算する）。アタリの候補魚は釣り場・餌に加え、**現在の時間帯・天候**でも絞り込まれる
  （`game/fishing.ts` の `candidateFish`。8章の時間帯・天候システムと連動）。待機中は
  `ui/WaterScene.tsx` が経過率に応じて魚影シルエットをウキへ近づける演出を行う。
- **アワセ**: 待機中(`waiting`)も画面(水面)をタップ可能にし、アタリが出る前にタップすると
  「早合わせ」で失敗になる。アタリ発生後は `HOOK_WINDOW_MS` 以内にタップすれば成功、それを過ぎると
  「遅すぎ」で失敗になる。早すぎ/遅すぎの両方向の失敗を `FishingScreen.tsx` の `missReason` で
  区別してメッセージを出し分ける。
...
- **キャストの飛距離ゲージ**（`game/fishing.ts` の `castGaugePosition` / `castQualityFromPosition` /
  `castQualityBiteMultiplier`。三角波で往復するゲージを中央付近で止めるほどアタリまでの待ち時間が
  短くなる）
- **アワセの「早すぎ失敗」判定**（`waiting`中も水面をタップ可能にし、アタリが出る前にタップすると
  「早合わせ」失敗になる。従来の「遅すぎ失敗」と合わせて両方向の失敗を実装）
- **待機中に魚影が近づいてくる演出**（`ui/WaterScene.tsx`。待ち時間の経過率に応じて魚影シルエット
  がウキへ近づく）
- **釣り画面(キャスト〜ファイト)のドット絵背景**（`ui/WaterScene.tsx`。村マップと同じ
  `game/pixelArt.ts` の海タイル・ウキ・魚影を使い、キャスト〜アタリ待ち〜ファイト中まで一貫した
  Canvas演出にした。魚自体はまだ絵文字のまま）
- 画面遷移時の軽いフェード演出（`App.tsx` の `screen-fade-in`、`index.css`）
...
- 噂システムの実質的な活用（`rumors` は保存されるが、解放条件としては未接続。現状のエリア解放は
  図鑑の捕獲済みフラグのみで判定している）
- 釣れた魚自体の見た目のドット絵化（現状は魚のみ絵文字のまま。水面・ウキ・魚影は`pixelArt.ts`製）
- セーブスロット複数化、実績等
...
物語（世界観・人物・章構成・エンディング・のめり込ませる仕組み）は
**[docs/story.md](./docs/story.md)** に分離して管理する。台詞・章進行フラグ・イベントを
実装するときは、まず `docs/story.md` を更新してから着手すること。

要点だけ抜き出すと:

- 主人公は、祖父 宗吉が遺した**空白だらけの海図**を持って凪浦にやってくる若者。
  **図鑑＝祖父の海図**であり、魚を釣ることがそのまま物語の前進になる。
- 4つのエリアのぬし（黒天→岩鬼→潮牙→海のぬし）が**そのまま章の区切り**。
  ぬしを釣るたびに20年前の嵐の夜の真相が一つずつ明らかになる（証拠アイテム: 錘→針→懐中時計）。
- 対立役は道具屋の店主 **辰次**。彼の態度の変化が章進行の可視化装置になる。
- 大ぬしを釣り上げた時、**「海へ還す／持ち帰る」の選択**でエンディングが分岐する。
  どちらも正解として書き、クリア後も釣りは続けられる。
- 未使用だった `SaveData.rumors` は、噂→ぬしの出現条件を海図に書き込む仕組みとして活用する
  （8章の「未実装」から story.md 側のロードマップへ移す想定）。
